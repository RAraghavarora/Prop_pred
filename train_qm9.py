...
def prepare_data(op, dataset_op):
    ...
    return np.array(reps2), np.array(TPROP2), slatm_len
...
class NeuralNetwork(nn.Module):
    def __init__(self, slatm_len, prop_len):
...
class EarlyStopping():
    ...
    def __init__(self, patience=7000, min_delta=0.005):
        """
...
def plotting_results(model, test_loader, fold, op):
    # applying nn model
    with torch.no_grad():
    ...
    plt.plot(temp, temp)
    plt.ylabel(f"True {op}")
    plt.xlabel(f"Predicted {op}")
...
def split_data(n_train, n_val, n_test, Repre, Target, seed):
    ...
    test_size = min(n - n_train - n_val, 10**4)
    ...
    for fold in range(0, split):
        ...
        for t in range(epochs):
            ...
            if early_stopping.early_stop:
                warnings.warn(
                    f"Stopping early after {t+1} epochs for {n_train}")
                break
...
op = 'EGAP'
dataset = 'qm9'
directory = 'withdft/slatm/egap/qm9/'

if dataset == 'qm9':
    train_set = [40000, 4000]
else:
    train_set = [i * 1000 for i in [30, 25]]
...
iX, iY, slatm_len = prepare_data(op, dataset)
